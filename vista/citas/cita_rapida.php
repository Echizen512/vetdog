          ...
          <div class="card">
            ...
            <div class="body">
              <form method="POST" autocomplete="off" id="form">
                <div class="row clearfix">
                  <!-- <div class="col-sm-12">
                    <label class="control-label">Diagnóstico<span class="text-danger">*</span></label>
                    <div class="form-group">
                      <div class="form-line">
                        <input type="text" name="diagnosis" class="form-control" placeholder="Diagnóstico de las mascota(s)..." required>
                      </div>
                    </div>
                  </div> -->
                  ...
                  <div class="col-sm-6" style="display: flex;flex-direction:column;gap:10px">
                    ...
                    foreach ($results as $key => $row) { ?>
                      <input type="checkbox" id="checkbox-<?php echo $key ?>" name="service" class="checkbox-services" value="<?php echo $row['id_servi'] ?>">
                      <label for="checkbox-<?php echo $key ?>"><?php echo $row['nomser'] ?></label>
                    <?php } ?>
                  </div>
                </div>
                ...
                <article class="row clearfix">
                  <div class="col-sm-6">
                    ...
                    <div class="input-group">
                      <div class="form-line">
                        <input type="text" required name="attended" class="form-control" placeholder="Atendido por...">
                      </div>
                    </div>
                  </div>
                ...
                <div class="container-fluid" align="center">
                  ...
                  <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3">
                    <button class="btn bg-blue" name="agregar">CREAR CONSULTA <i class="material-icons">save</i></button>
                  </div>
                </div>
              </form>
            </div>
...
// * Vertify if get All data for URL 
  addEventListener('load',() => {
    // Get all data the URL 
    const urlSearchParams = new URLSearchParams(location.search);
    const arrIdPets = urlSearchParams.getAll('pet_id');

    if(arrIdPets.length === 0 || arrIdPets[0] === '') return location.href = './nuevo.php';
  });
  ...
  form.addEventListener('submit',async e => {
    ...
    const urlSearchParams = new URLSearchParams(location.search);
    const arrIdPets = urlSearchParams.getAll('pet_id');
    //Count total the pets
    const numberPets = arrIdPets[0].split(',').length;
    ...
    const req = await fetch('./controllerRequest.php',{
      ...
      body: JSON.stringify({data,checkboxValues,arrIdPets,numberPets}),
    });

    const res = await req.json();

    if(res.success) {
      swal('¡Creado!','¡Consulta creada!','success').then(()=> {
        return location.href = '../../folder/citas';
      });
    }
  });
</script>
