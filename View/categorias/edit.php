...
<body class="theme-red">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <div class="card">
            ...
            <div class="body">
              <?php
              $id = $_GET['id'];
              $sql = "SELECT id_cate,nomcate FROM category WHERE id_cate = '$id'";
              $results = mysqli_query($conn,$sql);
              foreach($results as $row) { 
                ...
                <form method="POST" autocomplete="off">
                  <div class="row clearfix">
                    <div class="col-sm-12">
                      <label class="control-label">Nombre de la Categoría<span class="text-danger">*</span></label>
                      <div class="form-group">
                        <div class="form-line">
                          <input type="text" name="nomcate" value="<?= $row['nomcate'] ?>" class="form-control" placeholder="Nombre de la Categoría..." />
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  <?php
    ...
    echo $id_cate;
    $sql = "SELECT * FROM category WHERE id_cate != '$id_cate' AND nomcate = '$nomcate' ";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) { ?>
        <script>
          Swal.fire('¡Error!','Ya existe la categoría','error');
        </script>
  <?php
    } else {
      $sql = "UPDATE category SET nomcate = '$nomcate' WHERE id_cate = '$id_cate'";
      if (mysqli_query($conn, $sql)) { ?>
          <script>
            Swal.fire('¡Editado Exitosamente!','¡La categoría se ha editado correctamente!','success').then(() => {
        ...
        $rol = "administrador";
        $nameTable = "categoria";
        audit($id_cate,$nameTable,$adminID,$rol,$action);
        exit;
        } else { ?>
          <script>
            Swal.fire({
